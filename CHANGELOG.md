...
* resource/agentops_hosted_agent, resource/agentops_worker_catalog_deployment: Deleting an agent that deployed successfully now archives it first. On an account with hosted lifecycle enabled the API refuses the delete with `409 archive the agent before deleting it`, which left `terraform destroy` unable to remove what it had created.
